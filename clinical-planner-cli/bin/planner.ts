 ...
 * - research-plan-implement: Full RPI workflow with authoritative sources
...
// ============================================
// RESEARCH-PLAN-IMPLEMENT COMMAND (RPI Mode)
// ============================================
program
  .command('research-plan-implement')
  .alias('rpi')
  .description('Full workflow: research → plan → implement')
  .requiredOption('-c, --concern <id>', 'Concern ID (e.g., CLABSI, CAUTI, I06)')
  .option('-d, --domain <domain>', 'Override auto-detected domain (optional)')
  .option('-o, --output-dir <path>', 'Output directory for all artifacts')
  .option('--force-refresh', 'Force refresh cached research data')
  .option('--api-key <key>', 'OpenAI API key (or set OPENAI_API_KEY env var)')
  .action(async (options) => {
    try {
      // Auto-detect domain if not provided
      if (!options.domain) {
        options.domain = detectDomain(options.concern);
        console.log(`   ℹ️  Auto-detected domain: ${options.domain} - ${getDomainDescription(options.domain as any)}\n`);
      }
      await researchPlanImplementCommand(options);
    } catch (error: any) {
      console.error(`\n❌ Error: ${error.message}\n`);
      process.exit(1);
    }
...
// ============================================
// RESEARCH COMMAND (Research Only)
// ============================================
program
  .command('research')
  .description('Fetch research sources only (no plan generation)')
  .requiredOption('-c, --concern <id>', 'Concern ID (e.g., CLABSI, CAUTI)')
  .option('-d, --domain <domain>', 'Clinical domain')
  .option('-o, --output <path>', 'Output path for research bundle')
...
const cacheCmd = program
  .command('cache')
  .description('Manage research cache');

cacheCmd
  .command('list')
  .alias('ls')
  .description('List all cached research data')
  .action(async () => {
    try {
      await listCache({});
    } catch (error: any) {
...
cacheCmd
  .command('refresh')
  .description('Refresh cache for a specific concern')
  .requiredOption('-c, --concern <id>', 'Concern ID to refresh')
  .option('-s, --source <authority>', 'Specific source to refresh')
  .action(async (options) => {
    try {
