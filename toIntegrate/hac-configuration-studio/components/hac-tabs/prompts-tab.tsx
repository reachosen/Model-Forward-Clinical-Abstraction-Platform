...
import type { HACTaskPrompt } from "@/lib/types/hac-config"
...
function PromptEditor({ promptType, prompts, onChange }: PromptEditorProps) {
  ...
  const handleDuplicate = () => {
    ...
    const newPrompt: HACTaskPrompt = {
      ...
      version: `${Number.parseInt(activePrompt.version.split(".")[0]) + 1}.0.0`,
      status: "draft",
      is_active: false,
    }
  ...
  return (
    <div className="space-y-6">
      <Alert>
        <AlertCircle className="h-4 w-4" />
        <AlertDescription>
          Prompts define how the AI analyzes cases for this HAC. Each prompt type serves a different purpose in the
          pipeline.
        </AlertDescription>
      </Alert>
