...
import { SafetyGrader } from '../graders/SafetyGrader';
import { SchemaGrader } from '../graders/SchemaGrader';
import { ReportGenerator, EvalReport } from '../reporting/ReportGenerator';
...
export const evalCommand = new Command('eval')
  ...
  .option('--test-dir <path>', 'Test data directory (defaults to domain path for metric)')
  .action(async (options) => {
    try {
      const { metric, batch, mode, output } = options;

      // Resolve test directory: use provided path or derive from metric
      ...
      if (!testDir) {
        try {
          ...
          // Fallback to legacy path if metric not in registry
      ...
      for (const tc of testCases) {
        ...
        try {
          ...
          engineOutput = {
            summary: plan.clinical_config.summary_config?.event_summary || (plan as any).rationale?.summary,
            signals: (plan.clinical_config.signals.signal_groups || []).flatMap((g: any) => g.signals.map((s: any) => s.description || s.name)),
            followup_questions: questionsArray.map((q: any) => q.question_text || q.text),
            latency_ms: Date.now() - startTime,
            raw_input: (input as any).patient_payload || ''
          };
        } catch (err: any) {
