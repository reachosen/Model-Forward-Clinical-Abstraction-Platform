...
export class S0_InputNormalizationStage {
  ...
  async execute(input: PlanningInput): Promise<RoutedInput> {
    ...
    // Build routed input
    let patient_payload = input.clinical_context?.patient_payload ?? input.metadata?.notes;

    // Fallback: Synthesize narrative if specific context fields are provided
    if (!patient_payload && input.clinical_context) {
      const parts = [];
      if (input.clinical_context.admission_summary) {
        parts.push(`ADMISSION SUMMARY:\n${input.clinical_context.admission_summary}`);
      }
      if (input.clinical_context.timeline && input.clinical_context.timeline.length > 0) {
        const timelineStr = input.clinical_context.timeline
          .map((t: any) => `- [${t.timestamp}] ${t.event}`)
          .join('\n');
        parts.push(`TIMELINE:\n${timelineStr}`);
      }
      if (parts.length > 0) {
        patient_payload = parts.join('\n\n');
      }
    }
